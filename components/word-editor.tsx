...
export function WordEditor({ item, onClose, onSave, onDelete }: { item?: VocabularyItem; onClose: () => void; onSave: (item: VocabularyItem) => void; onDelete: (id: string) => void }) {
  ...
  const grammarForms = draft.grammarForms ?? { infinitive: draft.lemma ?? '', stem: '', meaning: draft.translation, firstPresent: draft.target, you: '', we: '', negativeI: '' };
  const updateGrammarForm = (key: keyof typeof grammarForms, value: string | boolean) => setDraft({ ...draft, partOfSpeech: 'verb', lemma: key === 'infinitive' && typeof value === 'string' ? value : draft.lemma, grammarForms: { ...grammarForms, [key]: value } });
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-[#082b28]/60 p-3 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div role="dialog" aria-modal="true" className="max-h-[94vh] w-full max-w-3xl overflow-y-auto rounded-3xl bg-card shadow-2xl">
        ...
        <div className="grid gap-5 p-5 sm:grid-cols-2">
          ...
          <label><span className="mb-2 block text-sm font-medium">Wortart</span><select className="h-9 w-full rounded-xl border bg-background px-3 text-sm" value={draft.partOfSpeech ?? 'other'} onChange={(event) => setDraft({ ...draft, partOfSpeech: event.target.value as VocabularyItem['partOfSpeech'] })}><option value="verb">Verb</option><option value="noun">Nomen</option><option value="pronoun">Pronomen</option><option value="phrase">Satz / Redewendung</option><option value="adverb">Zeit / Adverb</option><option value="other">Andere</option></select></label>
          ...
          {draft.partOfSpeech === 'verb' && !draft.verbProfileId && <div className="grid gap-4 rounded-2xl border border-primary/15 bg-primary/5 p-4 sm:col-span-2 sm:grid-cols-2"><div className="sm:col-span-2"><strong className="text-sm">Grammatikprofil für dieses Verb</strong><p className="mt-1 text-xs text-muted-foreground">Diese Formen erzeugen automatisch eigene Grammatik- und Satzkarten. Unregelmäßige Formen bitte ausschreiben.</p></div><Field label="Infinitiv" value={grammarForms.infinitive} onChange={(value) => updateGrammarForm('infinitive', value)} /><Field label="Stamm (z. B. -penda)" value={grammarForms.stem} onChange={(value) => updateGrammarForm('stem', value)} /><Field label="Ich-Form" value={grammarForms.firstPresent} onChange={(value) => updateGrammarForm('firstPresent', value)} /><Field label="Du-Form" value={grammarForms.you} onChange={(value) => updateGrammarForm('you', value)} /><Field label="Wir-Form" value={grammarForms.we} onChange={(value) => updateGrammarForm('we', value)} /><Field label="Negative Ich-Form" value={grammarForms.negativeI} onChange={(value) => updateGrammarForm('negativeI', value)} /></div>}
