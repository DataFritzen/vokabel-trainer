...
export function ProfileDashboard({ snapshot, words }: { snapshot: AppSnapshot; words: VocabularyItem[] }) {
  const masteredWords = words.filter((word) => word.card.state === 2 && word.card.reps >= 2).length;
  const vocabScore = words.length ? Math.round(masteredWords / words.length * 100) : 0;
  const practicedGrammar = snapshot.grammarExercises.filter((exercise) => exercise.card.reps > 0).length;
  const grammarScore = snapshot.grammarExercises.length ? Math.round(practicedGrammar / snapshot.grammarExercises.length * 100) : 0;
